...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZBALLModule::validateInput(txin, prevOut, tx, ret)){
...
bool RecalculateBALLSupply(int nHeightStart, bool fSkipZball)
{
    ...
    while (true) {
        ...
        // Rewrite zball supply too
        if (!fSkipZball && consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC)) {
