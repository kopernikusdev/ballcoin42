    ...
    if(IsCrypted()) {
        if (!IsLocked()) { //if we have password
            ...
            if (EncryptSecret(vMasterKey, kmSeed, hashSeed, vchSeedSecret)) {
                ...
                if (db.WriteZBALLSeed(hashSeed, vchSeedSecret)) {
                    return true;
                }
            }
            strErr = "encrypt seed";
        ...
        if (db.WriteZBALLSeed(hashSeed, ToByteVector(seed))) {
            return true;
        }
        strErr = "save zballseed to wallet";
    }
    //the use case for this is no password set seed, mint dzBALL,
...
bool CWallet::GetDeterministicSeed(const uint256& hashSeed, uint256& seedOut)
{
    ...
    if (IsCrypted()) {
        if(!IsLocked()) { //if we have password
            ...
            //read encrypted seed
            if (db.ReadZBALLSeed(hashSeed, vchCryptedSeed)) {
        ...
        // wallet not crypted
        if (db.ReadZBALLSeed(hashSeed, vchSeed)) {
...
void CWallet::doZBallRescan(const CBlockIndex* pindex, const CBlock& block,
        std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZBALL)
{
    //If this is a zapwallettx, need to read zball
    if (fCheckZBALL && consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC)) {
                                            ...
                                            const CCoinControl* coinControl)
{
    ...
    while (true) {
        ...
        if (!CreateZBALLOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zball output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to Ball
        ...
        std::list<std::pair<CTxDestination, CAmount>> addressesTo, CTxDestination* changeAddress)
{
    ...
    if (res.status != CWallet::CommitStatus::OK) {
        ...
        for (CZerocoinMint mint : vMintsSelected) {
            ...
            zballTracker->SetPubcoinNotUsed(hashPubcoin);
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zballTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zballTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
                                    ...
                                    CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    for (auto &it : mapMintsSelected) {
        ...
        if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
            receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), ZBALL_TXMINT_GENERAL);
            return false;
        } else if (mapBlockIndex.count(hashBlock) < 1) {
            // check that this mint made it into the blockchain
            receipt.SetStatus(_("Mint did not make it into blockchain"), ZBALL_TXMINT_GENERAL);
            return false;
        }
        ...
        if (outputIndex == -1) {
            receipt.SetStatus(_("Pubcoin not found in mint tx"), ZBALL_TXMINT_GENERAL);
            return false;
        }

        mint.SetOutputIndex(outputIndex);
        CTxIn in;
        if(!ZBALLModule::createInput(in, mint, hashTxOut, spendVersion)) {
            receipt.SetStatus(_("Cannot create public spend input"), ZBALL_TXMINT_GENERAL);
            return false;
        }
        vin.emplace_back(in);
        ...
        CTxDestination * changeAddress)
{
    ...
    const int nMaxSpends = Params().GetConsensus().ZC_MaxPublicSpendsPerTx; // Maximum possible spends for one zBALL public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zBALL used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        setMints = zballTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
    ...
    for (CZerocoinMint mint : vSelectedMints) {
        ...
        if (IsSerialInBlockchain(mint.GetSerialNumber(), nHeightSpend)) {
            ...
            if (!zballTracker->HasSerialHash(hashSerial))
                return error("%s: tracker does not have serialhash %s", __func__, hashSerial.GetHex());

            CMintMeta meta = zballTracker->Get(hashSerial);
            meta.isUsed = true;
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zBALL in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zballTracker->GetMints(true);
    ...
    return zballTracker->GetBalance(false, false);
}

CAmount CWallet::GetUnconfirmedZerocoinBalance() const
{
    return zballTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zballTracker->ListMints(true, true, true);
...
void CWallet::setZWallet(CzBALLWallet* zwallet)
{
    zwalletMain = zwallet;
    zballTracker = std::unique_ptr<CzBALLTracker>(new CzBALLTracker(this));
}

CzBALLWallet* CWallet::getZWallet()
{
    return zwalletMain;
}
...
{
    return zballTracker->HasSerial(bnSerial);
}

bool CWallet::IsMyMint(const CBigNum& bnValue) const
{
    if (zballTracker->HasPubcoin(bnValue))
        return true;
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zballTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zballTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zballTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
...
{
    CMintMeta meta;
    if (!zballTracker->GetMetaFromStakeHash(hashStake, meta))
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zballTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zballTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zballTracker->UpdateState(meta);
    } else {
