...
#include "zball/zballmodule.h"
#include "zball/zballwallet.h"
#include "zball/zballtracker.h"
...
enum WalletFeature {
    ...
    FEATURE_PRE_BALLCOIN = 61000, // inherited version..
...
// Possible states for zBALL send
enum ZerocoinSpendStatus {
    ZBALL_SPEND_OKAY = 0,                            // No error
    ZBALL_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBALL_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBALL_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBALL_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBALL_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBALL_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBALL_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBALL_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBALL_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZBALL_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBALL_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBALL_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBALL_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBALL_SPENT_USED_ZBALL = 14,                      // Coin has already been spend
    ZBALL_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZBALL_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    /// Get 10000 BALL output and keys which can be used for the Masternode
    ...
    void doZBallRescan(const CBlockIndex* pindex, const CBlock& block, std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZBALL);
    ...
    bool CreateZBALLOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zBALL wallet
    CzBALLWallet* zwalletMain{nullptr};
    std::unique_ptr<CzBALLTracker> zballTracker{nullptr};
    void setZWallet(CzBALLWallet* zwallet);
    CzBALLWallet* getZWallet();
    ...
    // zBALL reset
    boost::signals2::signal<void()> NotifyzBALLReset;
