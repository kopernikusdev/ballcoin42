...
// Copyright (c) 2016-2020 The BALLCOIN developers
...
public:
    ...
    bool WriteZBALLSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZBALLSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZBALLSeed_deprecated(uint256& seed);
    bool EraseZBALLSeed();
    bool EraseZBALLSeed_deprecated();

    bool WriteZBALLCount(const uint32_t& nCount);
    bool ReadZBALLCount(uint32_t& nCount);
