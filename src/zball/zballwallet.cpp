// Copyright (c) 2017-2020 The BALLCOIN developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zballwallet.h"
#include "main.h"
#include "txdb.h"
...
CzBALLWallet::CzBALLWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing zball seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBALLSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBALLSeed_deprecated()) {
                    LogPrintf("%s: Updated zBALL seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zball seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zball wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBALLWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBALLCount(nCountLastUsed);
    else if (!walletdb.ReadZBALLCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBALLWallet::Lock()
{
    seedMaster.SetNull();
}

void CzBALLWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBALLWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBALL(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBALLWallet::LoadMintPoolFromDB()
{
...
void CzBALLWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBALLWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBALLWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zballTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBALLWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBALL(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zballTracker which also adds to database
    wallet->zballTracker->Add(dMint, true);
...
void CzBALLWallet::SeedToZBALL(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzBALLWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBALLWallet::GenerateDeterministicZBALL(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBALLWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBALLWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBALLWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
