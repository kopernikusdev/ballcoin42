// Copyright (c) 2018-2020 The BALLCOIN developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zball/deterministicmint.h>
#include "zballtracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
#include "zball/zballwallet.h"


CzBALLTracker::CzBALLTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzBALLTracker::~CzBALLTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBALLTracker::Init()
{
...
bool CzBALLTracker::Archive(CMintMeta& meta)
{
...
bool CzBALLTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBALLTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBALLTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBALLTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzBALLTracker::GetSerialHashes()
{
...
bool CzBALLTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBALLTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBALLTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBALLTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBALLTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBALLTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBALLTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzBALLTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBALLWallet* zBALLWallet)
{
    bool iszBALLWalletInitialized = (nullptr != zBALLWallet);
    CMintMeta meta;
    ...
    if (!iszBALLWalletInitialized)
        zBALLWallet = new CzBALLWallet(wallet);
    meta.isSeedCorrect = zBALLWallet->CheckSeed(dMint);
    if (!iszBALLWalletInitialized)
        delete zBALLWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBALLTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBALLTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBALLTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBALLTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBALLTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBALLTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzball from DB\n", __func__, listDeterministicDB.size());
    }
