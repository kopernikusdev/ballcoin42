...
bool CBallStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CBallStake from zerocoin spend", __func__);
...
bool CBallStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CBallStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CBallStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CBallStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CBallStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CBallStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CBallStake::GetUniqueness() const
{
    //The unique identifier for a BALL stake is the outpoint
...
CBlockIndex* CBallStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CBallStake::ContextCheck(int nHeight, uint32_t nTime)
{
