...
#include "qt/ballcoin/ballcoingui.h"
...
#include "qt/ballcoin/splash.h"
#include "qt/ballcoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("ballcoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in ballcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in ballcoin.qrc)
...
/** Class encapsulating BALLCOIN Core startup and shutdown.
...
/** Main BALLCOIN application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (BALLCOINGUI)
...
{
    window = new BALLCOINGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &BALLCOINGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &BALLCOINGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(BALLCOINGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(BALLCOINGUI::DEFAULT_WALLET);
        ...
        // BALLCOIN: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BALLCOINGUI::handlePaymentRequest);
        connect(window, &BALLCOINGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. BALLCOIN can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(ballcoin_locale);
    Q_INIT_RESOURCE(ballcoin);
    ...
    /// 6. Determine availability of data directory and parse ballcoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("BALLCOIN Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("BALLCOIN Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // ballcoin: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("BALLCOIN Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
