...
#include "qt/ballcoin/topbar.h"
#include "qt/ballcoin/forms/ui_topbar.h"
#include "qt/ballcoin/lockunlock.h"
#include "qt/ballcoin/qtutils.h"
#include "qt/ballcoin/receivedialog.h"
#include "qt/ballcoin/loadingdialog.h"
...
TopBar::TopBar(BALLCOINGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopBall}, "amount-small-topbar");
    setCssProperty({ui->labelAmountBall}, "amount-topbar");
    setCssProperty({ui->labelPendingBall, ui->labelImmatureBall}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("BALLCOIN")
    updateDisplayUnit();
                            ...
                            const CAmount& delegatedBalance, const CAmount& coldStakedBalance)
{
    ...
    // BALL Total
    QString totalBall = GUIUtil::formatBalance(balance, nDisplayUnit);

    // BALL
    // Top
    ui->labelAmountTopBall->setText(totalBall);
    // Expanded
    ui->labelAmountBall->setText(totalBall);
    ui->labelPendingBall->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelImmatureBall->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
}
