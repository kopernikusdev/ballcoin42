...
#include "qt/ballcoin/dashboardwidget.h"
#include "qt/ballcoin/forms/ui_dashboardwidget.h"
#include "qt/ballcoin/sendconfirmdialog.h"
#include "qt/ballcoin/txrow.h"
...
DashboardWidget::DashboardWidget(BALLCOINGUI* parent) :
    PWidget(parent),
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &BALLCOINGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &BALLCOINGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &BALLCOINGUI::openFAQ);
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isBall = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZBALL;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair BALL, zBALL
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesBall);
    set1->append(chartData->valueszBall);
    ...
    ui->labelAmountBall->setText(GUIUtil::formatBalance(chartData->totalBall, nDisplayUnit));
    ui->labelAmountZball->setText(GUIUtil::formatBalance(chartData->totalZball, nDisplayUnit, true));
