...
Auto Wallet Backup
---------------------
In addition to the automatic wallet backup that is done at each start of the client, a new automatic backup function has been added that will, by default, create a backup of the wallet file during each zBALL mint operation (zBALL spends which re-mint their change are also included in this). This functionality is controlled by the `-backupzball` command-line option, which defaults to `1` (enabled, auto-backup).

Users that wish to prevent this behavior (not recommended) can pass `-backupzball=0` at the command-line when starting the client, or add `backupzball=0` to their `ballcoin.conf` file.
...
- #292 `feadab4` Additional checks for double spending of zBall serials. (presstab)

### Wallet
- #271 `5e9a086` [Wallet] Remove unused member wallet in UnlockContext inner class (Jon Spock)
- #279 `e734010` Add -backupzball startup flag. (presstab)
- #280 `fdc182d` [Wallet] Fix zBall spending errors. (presstab)
- #282 `310f216` [Wallet] Count pending zBall balance for automint. (presstab)
...
- #291 `cb314e6` [Qt] zBall control quantity/amount fixes (rejectedpromise)
